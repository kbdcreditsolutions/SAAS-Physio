...
export async function GET(req: NextRequest) {
  ...
  const staffStatus = new Map(records.filter((r) => r.userId).map((r) => [r.userId, r.status]));
  const patientStatus = new Map(records.filter((r) => r.patientId).map((r) => [r.patientId, r.status]));
